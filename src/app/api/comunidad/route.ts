...
// GET — lista los temas del foro con la cantidad de respuestas.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { categoria, titulo, cuerpo, autor, autor_email } = body;
    ...
    `;
    return NextResponse.json({ ...rows[0], respuestas: 0 }, { status: 201 });
  } catch (error) {
